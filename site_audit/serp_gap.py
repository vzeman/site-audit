...
function pointDetail(p){const type=p.entity_type||'point';let explanation;if(type==='keyword')explanation='Search query vector used as the semantic anchor for this SERP comparison.';else if(p.source==='ours')explanation=`Our ${type}: content from the audited page. Distance from competitor clusters shows how closely it matches the SERP topic space.`;else if(p.source==='competitor')explanation=`Competitor ${type}: extracted from a ranking SERP page. Dense nearby competitor points indicate topics common in top results.`;else explanation=`${type}: semantic reference point in this keyword analysis.`;return{type,source:p.source||'',cluster:p.cluster??'',domain:p.domain||'',rank:p.rank||'',url:p.url||'',text:String(p.text||'').slice(0,320),explanation};}
function pointTooltip(p){const d=pointDetail(p);return [d.explanation,`Cluster: ${d.cluster||'n/a'}`,d.domain&&`Domain: ${d.domain}`,d.rank&&`SERP rank: ${d.rank}`,d.url&&`URL: ${d.url}`,d.text&&`Text: ${d.text}`].filter(Boolean).join('\\n');}
function pointDetailHtml(d){const sourceClass=d.type==='keyword'?'keyword':d.source==='ours'?'ours':d.source==='competitor'?'competitor':'';const meta=[['Cluster',d.cluster||'n/a'],['Source',d.source||'semantic'],['Domain',d.domain],['SERP rank',d.rank],['URL',d.url]].filter(row=>row[1]);return `<div class="tip-head"><div><div class="tip-title">${esc(d.type)} point</div><div class="tip-sub">Semantic scatterplot detail</div></div><button class="tip-close" type="button" aria-label="Close tooltip">×</button></div><div class="tip-body"><div class="tip-badges"><span class="tip-badge ${sourceClass}">${esc(d.source||d.type)}</span><span class="tip-badge">cluster ${esc(d.cluster||'n/a')}</span>${d.rank?`<span class="tip-badge">rank ${esc(d.rank)}</span>`:''}</div><div class="tip-explain">${esc(d.explanation)}</div><div class="tip-grid">${meta.map(([k,v])=>`<div class="tip-field"><span>${esc(k)}</span><strong>${esc(v)}</strong></div>`).join('')}</div>${d.text?`<div class="tip-text">${esc(d.text)}</div>`:''}</div>`;}
