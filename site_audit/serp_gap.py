...
function sourceColor(p){if(p.entity_type==='keyword')return colors.keyword;const domain=pointDomain(p);if(domain)return domainColor(domain);if(p.entity_type==='title')return colors.title;if(/^h[1-6]$/.test(p.entity_type||''))return colors.h1;if(p.entity_type==='header')return colors.header;return colors.competitor;}
...
function serpUrlGraph(rows){if(!rows||rows.length<2)return '<div class="empty">Not enough URLs for a co-ranking graph.</div>';const nodes=rows.slice(0,18);const edges=[];for(let i=0;i<nodes.length;i++){for(let j=i+1;j<nodes.length;j++){const shared=sharedKeywordNames(nodes[i],nodes[j]);if(shared.length)edges.push({a:i,b:j,weight:shared.length,keywords:shared});}}if(!edges.length)return '<div class="empty">No URLs share selected keywords.</div>';const w=920,h=460,cx=w/2,cy=h/2,r=Math.min(w,h)*0.36;const maxCount=Math.max(1,...nodes.map(row=>Number(row.top10_count||0)));const positions=nodes.map((node,i)=>{const angle=-Math.PI/2+(i/nodes.length)*Math.PI*2;return{x:cx+Math.cos(angle)*r,y:cy+Math.sin(angle)*r};});const edgeSvg=edges.map(edge=>{const a=positions[edge.a],b=positions[edge.b];const title=`${nodes[edge.a].url} ↔ ${nodes[edge.b].url}: ${edge.weight} shared keyword${edge.weight===1?'':'s'} (${edge.keywords.join(', ')})`;return `<line class="graph-edge" x1="${a.x.toFixed(1)}" y1="${a.y.toFixed(1)}" x2="${b.x.toFixed(1)}" y2="${b.y.toFixed(1)}" stroke-width="${(1.2+edge.weight*1.8).toFixed(1)}"><title>${esc(title)}</title></line>`;}).join('');const nodeSvg=nodes.map((node,i)=>{const p=positions[i];const count=Number(node.top10_count||0);const size=8+Math.sqrt(count/maxCount)*12;const domain=node.domain||urlDomain(node.url||'')||node.url;const title=`${node.url}\\n${count} top-10 appearance${count===1?'':'s'}\\nBest rank #${node.best_rank||''}\\nKeywords: ${(node.keywords||[]).map(k=>`${k.keyword} #${k.rank}`).join(', ')}`;const labelY=p.y+size+14;return `<a href="${esc(node.url||'#')}" target="_blank" rel="noopener noreferrer"><circle class="graph-node" cx="${p.x.toFixed(1)}" cy="${p.y.toFixed(1)}" r="${size.toFixed(1)}" fill="${domainColor(domain)}"><title>${esc(title)}</title></circle></a><text class="graph-label" x="${p.x.toFixed(1)}" y="${labelY.toFixed(1)}" text-anchor="middle">${esc(domain).slice(0,28)}</text><text class="graph-meta" x="${p.x.toFixed(1)}" y="${(labelY+13).toFixed(1)}" text-anchor="middle">${n(count)} top-10 · best #${esc(node.best_rank||'')}</text>`;}).join('');return `<svg class="serp-url-graph" viewBox="0 0 ${w} ${h}" role="img" aria-label="URL co-ranking graph. Nodes are ranking URLs, and line thickness increases when URLs rank for more of the same selected keywords."><rect x="0" y="0" width="${w}" height="${h}" fill="transparent"></rect>${edgeSvg}${nodeSvg}</svg><div class="mini">Co-ranking graph: nodes are URLs; connected URLs rank for the same selected keyword; thicker lines mean more shared keywords.</div>`;}
...
function overviewSection(){const points=data.overview_scatter?.points||[];const rankings=data.serp_url_rankings||[];return `<section class="page-section" id="overview-section"><div class="page-head"><h2>Keyword and Content Semantic Map</h2><div class="mini">All selected keywords, processed URLs, titles, headings, and paragraphs in one shared vector space.</div></div><div class="keyword-card"><div class="panel"><h4>Top-10 URLs Across Selected Keywords</h4><div class="panel-body">${serpUrlGraph(rankings)}${serpRankingChart(rankings)}${serpRankingList(rankings)}</div></div><div class="panel" style="margin-top:14px"><h4>All Keywords, URLs, and Content</h4><div class="panel-body">${scatterSvg(points)}</div></div></div></section>`;}
