...
function keywordMetrics(k){const parts=[`#${esc(k.rank)}`];parts.push(`${n(k.impressions||0)} impr`);parts.push(`${n(k.clicks||0)} clicks`);parts.push(`${n(k.traffic||0)} traffic`);if(Number(k.source_position||0))parts.push(`source pos ${Number(k.source_position).toFixed(1)}`);if(Number(k.volume||0))parts.push(`vol ${n(k.volume)}`);return parts.join(' · ');}
...
function graphTooltipHtml(detail){if(detail.kind==='connection'){return `<h5>Shared keyword connection</h5><div>${urlLink(detail.url_a,detail.domain_a)} ↔ ${urlLink(detail.url_b,detail.domain_b)}</div><div class="metric-line">Traffic proxy: ${Math.round(Number(detail.traffic_proxy||0)*100)}% · Shared keywords: ${n(detail.shared_count||0)}</div>${graphKeywordRows(detail.keywords)}`;}return `<h5>${esc(detail.domain||'URL')}</h5><div>${urlLink(detail.url)}</div><div class="metric-line">Top-10 keywords: ${n(detail.top10_count||0)} · Best SERP position: #${esc(detail.best_rank||'')} · Avg SERP position: #${esc(detail.average_rank||'')}</div><div class="metric-line">Impressions: ${n(detail.impressions||0)} · Clicks: ${n(detail.clicks||0)} · Traffic: ${n(detail.traffic||0)}</div>${graphKeywordRows(detail.keywords)}`;}
