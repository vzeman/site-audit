...
def build_technical_seo(
    ...
    *,
    ...
    for error in (sitemap_coverage or {}).get("sitemap_errors") or []:
        url = error.get("sitemap_url") or error.get("url") or ""
...
def _merge_page_signals(
    ...
    if search:
        ...
        row["previous_traffic"] = _safe_int(search.get("previous_traffic"))
    ...
    if media_issues:
        ...
        http_images = [
            ...
            for issue in (media_issues.get("https_page_links_to_http_image") or [])
        ]
        if http_images:
    ...
    if resource_issues:
        ...
        large_css = [
            ...
            for issue in (resource_issues.get("css_file_size_too_large") or [])
        ]
        if large_css:
    ...
    if sitemap:
        ...
        if "5xx_page_in_sitemap" in row["sitemap_issue_types"]:
        ...
        if "indexable_page_not_in_sitemap" in row["sitemap_issue_types"]:
...
def _issues_for_row(row: dict) -> list[dict]:
    ...
    if _safe_int(row.get("sitemap_5xx_count")) > 0:
    ...
    if _safe_int(row.get("sitemap_indexable_missing_count")) > 0:
...
def _recommendation(issue_type: str) -> str:
    return {
        ...
        "no_of_urls_in_sitemap_decreased": "Review the sitemap URL decrease and confirm the removed URLs were intentionally dropped.",
...
def _has_double_slash_in_url(url: str) -> bool:
    try:
        parsed = urlparse(url or "")
    except Exception:
        return False
...
def _organic_traffic_dropped(row: dict) -> bool:
    previous = _safe_int(row.get("previous_traffic"))
    current = _safe_int(row.get("traffic"))
    return previous > 0 and current < previous
...
def _search_lookup(payload: dict | None) -> dict[str, dict]:
    ...
    for row in (payload or {}).get("top_pages") or []:
        ...
        out[str(url)] = {
            ...
            "previous_traffic": row.get("previous_traffic", row.get("traffic_before", 0)),
