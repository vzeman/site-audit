...
def _task_markdown(page: dict, schema_doc: str) -> str:
    return "\n".join([
        ...
        "- If evidence.json contains `intent.match: mismatch`, address that first in",
        "  `page_assessment.reason`: say whether to retarget this page, create a new page,",
        "  or proceed despite the mismatch, with evidence.",
        "- If evidence.json contains `winnability.band: unlikely`, state that content changes",
        "  alone are unlikely to reach page 1; name the easier variant keyword if supplied",
